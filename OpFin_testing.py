...
# Expanded Asset Universe (Approximating "Whole Market" via top liquid assets)
ASSET_CLASSES = {
    "USA (USD)": {
        "Indices": ["SPY", "QQQ", "IWM", "VTI", "VOO", "DIA", "EFA", "EEM"],
        "Sector ETFs": ["XLK", "XLV", "XLF", "XLE", "XLC", "XLY", "XLP", "XLI", "XLB", "XLRE", "XLU", "SMH", "IBB", "KBE", "XRT"],
        "Stocks": [
            "AAPL", "MSFT", "GOOG", "AMZN", "NVDA", "TSLA", "META", "BRK-B", "UNH", "JNJ", 
            "JPM", "XOM", "V", "PG", "MA", "HD", "CVX", "MRK", "ABBV", "PEP", 
            "KO", "LLY", "BAC", "AVGO", "TMO", "COST", "DIS", "PFE", "CSCO", "ACN", 
            "MCD", "ADBE", "WFC", "LIN", "AMD", "CRM", "NFLX", "NKE", "ORCL", "QCOM",
            "IBM", "GE", "CAT", "BA", "MMM", "HON", "GS", "MS", "INTC", "TXN"
        ],
        "Bonds": ["SHY", "IEF", "TLT", "LQD", "HYG", "BND", "BNDX", "MUB", "TIP", "EMB"],
        "Commodities": ["GLD", "IAU", "SLV", "USO", "UNG", "DBC", "GSG", "PPLT", "PALL", "DBA"],
        "Crypto": ["BTC-USD", "ETH-USD", "SOL-USD", "BNB-USD", "XRP-USD", "ADA-USD", "DOGE-USD", "DOT-USD", "LTC-USD", "LINK-USD"]
    },
    "India (INR)": {
        "Indices": ["NIFTYBEES.NS", "JUNIORBEES.NS", "BANKBEES.NS", "MID150BEES.NS", "SDL26BEES.NS", "ITBEES.NS", "PHARMABEES.NS", "AUTOBEES.NS"],
        "Stocks": [
            "RELIANCE.NS", "TCS.NS", "HDFCBANK.NS", "ICICIBANK.NS", "INFY.NS", "HUL.NS", "ITC.NS", "SBIN.NS", "BHARTIARTL.NS", "KOTAKBANK.NS",
            "LT.NS", "AXISBANK.NS", "ASIANPAINT.NS", "HCLTECH.NS", "MARUTI.NS", "TITAN.NS", "BAJFINANCE.NS", "SUNPHARMA.NS", "ULTRACEMCO.NS", "NESTLEIND.NS",
            "ONGC.NS", "WIPRO.NS", "TATAMOTORS.NS", "POWERGRID.NS", "NTPC.NS", "ADANIENT.NS", "M&M.NS", "JSWSTEEL.NS", "TATASTEEL.NS", "LTIM.NS",
            "COALINDIA.NS", "HDFCLIFE.NS", "SBILIFE.NS", "BAJAJFINSV.NS", "BPCL.NS", "BRITANNIA.NS", "GRASIM.NS", "INDUSINDBK.NS", "CIPLA.NS", "DRREDDY.NS",
            "EICHERMOT.NS", "DIVISLAB.NS", "HEROMOTOCO.NS", "TATACONSUM.NS", "APOLLOHOSP.NS", "UPL.NS", "TECHM.NS", "HINDALCO.NS"
        ],
        "Commodities": ["GOLDBEES.NS", "SILVERBEES.NS"],
        "Bonds": ["LIQUIDBEES.NS", "GILT5YBEES.NS", "SETF10GILT.NS", "ICICILIQ.NS"]
    }
}
...
# Logic for importing top N assets
import_mode = st.sidebar.radio("Asset Selection Mode", ["Custom List", "Top N by CAGR (Market Index)"])

if import_mode == "Custom List":
    ...
    selected_tickers_for_opt = [t.strip() for t in ticker_input.split(',')]

else:
    # "Top N by CAGR" mode
    top_n = st.sidebar.number_input("Import Top N Assets", min_value=5, max_value=200, value=30)
    st.sidebar.info(f"Will select top {top_n} assets by 5Y CAGR from predefined universe.")
    selected_tickers_for_opt = [] # Will be populated after fetching market index
...
# DEFINE TABS IMMEDIATELY (MOVED OUTSIDE)
...
# --- TAB 1: ASSET INDEX (Always Visible & Populated with Defaults) ---
with tab1:
    st.subheader("Complete Asset Index")
    st.write("Browse and filter all available assets in the selected market.")
    
    # 1. Gather all default tickers for the market
    market_assets = ASSET_CLASSES.get(market_choice, {})
    all_market_tickers = []
    
    # Iterate through dictionary to get flat list
    for cat, t_list in market_assets.items():
        if isinstance(t_list, list):
             all_market_tickers.extend(t_list)
        elif isinstance(t_list, dict): # Handle if nested structure
             for subcat, sublist in t_list.items():
                 all_market_tickers.extend(sublist)
    
    # 2. Fetch data for them (Cached)
    with st.spinner("Loading Market Index..."):
         # We need to format them first
        fmt_market_tickers = [format_ticker(t, config['suffix']) for t in all_market_tickers]
        # Reuse fetch function
        _, idx_fundamentals, _, _ = fetch_financial_data(fmt_market_tickers, config)
    
    # 3. Display
    if idx_fundamentals is not None and not idx_fundamentals.empty:
        ...
        sorted_index_df = idx_fundamentals.sort_values(by=sort_col_index, ascending=ascending_index)
        ...
        # Logic for "Top N" selection mode (Must be here to access idx_fundamentals)
        if import_mode == "Top N by CAGR (Market Index)":
            # Select top N tickers sorted by CAGR descending
            top_assets = idx_fundamentals.sort_values(by="CAGR 5Y", ascending=False).head(top_n)
            selected_tickers_for_opt = top_assets['Ticker'].tolist()
            st.success(f"Auto-selected {len(selected_tickers_for_opt)} top assets by CAGR for optimization.")
            # Display the selected list
            st.write("Selected Assets for Optimization:")
            st.dataframe(top_assets[['Ticker', 'Name', 'CAGR 5Y']], use_container_width=True)

    else:
        st.error("Could not load market index.")

if 'data_download' not in st.session_state:
    st.session_state['data_download'] = None

# FETCH DATA BUTTON (Sidebar)
if st.sidebar.button("🚀 Load Data & Analyze"):
    # Use the tickers determined by the selection mode
    fmt_tickers = [format_ticker(t, config['suffix']) for t in selected_tickers_for_opt]
...
# --- DISPLAY TABS 2-6 (Only if data is loaded) ---
if st.session_state.get('prices') is not None:
    ...
    # Tabs 2-6 show a message if data hasn't been loaded via sidebar
    with tab2:
        st.info("Please enter tickers and click 'Load Data & Analyze' in the sidebar to unlock this tab.")
    with tab3:
        st.info("Please enter tickers and click 'Load Data & Analyze' in the sidebar to unlock this tab.")
    with tab4:
        st.info("Please enter tickers and click 'Load Data & Analyze' in the sidebar to unlock this tab.")
